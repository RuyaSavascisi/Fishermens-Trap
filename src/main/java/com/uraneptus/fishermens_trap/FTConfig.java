...
public class FTConfig {
    ...
    public static final ForgeConfigSpec.ConfigValue<Boolean> FULL_STACK_CATCH;
    ...
    static {
        ...
        MAX_TICKS_TO_FISH = COMMON_BUILDER.comment("The maximum amount of ticks that may pass until the next item can be fished by the Fish Trap. Value must be above min value. (default = 8000)").define("max ticks", 8000);
        ...
        FULL_STACK_CATCH = COMMON_BUILDER.comment("Makes the fish trap able to catch up to a full stack of fish per slot").define("enable full stack catch", false);
        COMMON_BUILDER.pop();
